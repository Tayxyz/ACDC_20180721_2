...
class dl():
    ...
    def write_sysenv(self,argv):
        self.env_cmds = {
                         ...
                         'TEST_WRITE_CONFIG_SWITCH_MLB': 'sysenv set  mlbconfig %s\r'%DATA.mlbconfig,
                         'TEST_WRITE_BUILDEVENT_SWITCH_MLB': 'sysenv set build_event EVT\r',
                         'TEST_WRITE_ISN_SWITCH_MLB': 'sysenv set mlb# %s\r'%DATA.isn,
                         'TEST_WRITE_NLWEAVEDEVICEID_SWITCH_MLB': 'sysenv set nlWeaveDeviceId %s\r'%DATA.MAC_6LOWPAN,
                         'TEST_WRITE_NLWEAVECERTIFICATE_SWITCH_MLB': 'sysenv set nlWeaveCertificate %s\r'%DATA.nlWeaveCertificate,
                         'TEST_WRITE_NLWEAVEPRIVATEKEY_SWITCH_MLB': 'sysenv set nlWeavePrivateKey %s\r' % DATA.nlWeavePrivateKey,
                         'TEST_WRITE_NLWEAVEPAIRINGCODE_SWITCH_MLB': 'sysenv set nlWeavePairingCode %s\r' % DATA.nlWeavePairingCode,
                         'TEST_WRITE_NLWEAVEPROVISIONINGHASH_SWITCH_MLB': 'sysenv set nlWeaveProvisioningHash %s\r' % DATA.nlWeaveProvisioningHash
                         }
        self.io._connect()
    ...
    def read_env(self,argv):
        ...
        self.env_kv = {'TEST_READ_NLMODEL_SWITCH_MLB': 'Tiros-1.1',
                         'TEST_READ_CONFIG_SWITCH_MLB': DATA.mlbconfig,
                         'TEST_READ_BUILDEVENT_SWITCH_MLB': 'EVT',
                         'TEST_READ_ISN_SWITCH_MLB': DATA.isn,
                         'TEST_READ_NLWEAVEDEVICEID_SWITCH_MLB': DATA.MAC_6LOWPAN,
                         'TEST_READ_NLWEAVECERTIFICATE_SWITCH_MLB': DATA.nlWeaveCertificate,
                         'TEST_READ_NLWEAVEPRIVATEKEY_SWITCH_MLB': DATA.nlWeavePrivateKey,
                         'TEST_READ_NLWEAVEPAIRINGCODE_SWITCH_MLB': DATA.nlWeavePairingCode,
                         'TEST_READ_NLWEAVEPROVISIONINGHASH_SWITCH_MLB': DATA.nlWeaveProvisioningHash
                         }
